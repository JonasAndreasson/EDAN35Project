                           ...
                           bool generate_mipmap)
{
	...
	auto data_pos_x = getTextureData(posx, width, height, false);
	if (data_pos_x.empty()) {
		glDeleteTextures(1, &texture);
		return 0u;
	}
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X,
		...
		/* the format of the pixel data: which components are available */GL_RGBA,
		/* the type of each component */GL_UNSIGNED_BYTE,
		/* the pointer to the actual data on the CPU */reinterpret_cast<GLvoid const*>(data_pos_x.data()));


	auto data_pos_y = getTextureData(posy, width, height, false);
	if (data_pos_y.empty()) {
		glDeleteTextures(1, &texture);
		return 0u;
	}
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y,
		...
		/* the format of the pixel data: which components are available */GL_RGBA,
		/* the type of each component */GL_UNSIGNED_BYTE,
		/* the pointer to the actual data on the CPU */reinterpret_cast<GLvoid const*>(data_pos_y.data()));

	auto data_neg_y = getTextureData(negy, width, height, false);
	if (data_neg_y.empty()) {
		glDeleteTextures(1, &texture);
		return 0u;
	}
	...
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y,
		...
		/* the format of the pixel data: which components are available */GL_RGBA,
		/* the type of each component */GL_UNSIGNED_BYTE,
		/* the pointer to the actual data on the CPU */reinterpret_cast<GLvoid const*>(data_neg_y.data()));

	auto data_pos_z = getTextureData(posz, width, height, false);
	if (data_pos_z.empty()) {
		glDeleteTextures(1, &texture);
		return 0u;
	}
	...
	glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z,
		...
		/* the format of the pixel data: which components are available */GL_RGBA,
		/* the type of each component */GL_UNSIGNED_BYTE,
		/* the pointer to the actual data on the CPU */reinterpret_cast<GLvoid const*>(data_pos_z.data()));

	auto data_pos_negz = getTextureData(negz, width, height, false);
	if (data_pos_negz.empty()) {
		glDeleteTextures(1, &texture);
		return 0u;
	}
	glTexImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z,
		...
		/* the format of the pixel data: which components are available */GL_RGBA,
		/* the type of each component */GL_UNSIGNED_BYTE,
		/* the pointer to the actual data on the CPU */reinterpret_cast<GLvoid const*>(data_pos_negz.data()));
