...
edan35::Assignment2::run()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (!shader_reload_failed) {
			...
			glViewport(0, 0, framebuffer_width, framebuffer_height);
			glClear(GL_DEPTH_BUFFER_BIT|GL_COLOR_BUFFER_BIT); //Color aswell seems to resolve the fragmentation.
		...
		if (show_textures) {
			...
			bonobo::displayTexture({ -0.95f,  0.55f }, { -0.55f,  0.95f }, textures[toU(Texture::GBufferDiffuse)], samplers[toU(Sampler::Linear)], { 0, 1, 2, -1 }, glm::uvec2(framebuffer_width, framebuffer_height));
...
FBOs createFramebufferObjects(Textures const& textures)
{
	...
	validate_fbo("GodRays");
	utils::opengl::debug::nameObject(GL_FRAMEBUFFER, fbos[toU(FBO::GodRays)], "GodRays");
